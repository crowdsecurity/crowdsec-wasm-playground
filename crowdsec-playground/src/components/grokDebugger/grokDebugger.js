...
function renderPattern(end_idx, pattern, submatch_idx) {
    ...
    for (let i = 0; i < pattern.length; i++) {
        if (i > end_idx) {
            patternStyles.push({
                text: pattern[i],
                style: { color: 'render_text_color', fontFamily: render_family_font },
            });
            continue;
        }
        ...
        if (insubmatch === true) {
            patternStyles.push({
                text: pattern[i],
                style: { color: colorFromKey(submatch_name), fontFamily: render_family_font },
            });
            continue;
        }

        //we're not in a submatch, but we're matched
        patternStyles.push({ text: pattern[i], style: { color: render_text_color, fontFamily: render_family_font } });
    }
    ...
    nextchar: for (let i = 0; i < text.length; i++) {
        ...
        for (const [k, indexes] of Object.entries(submatch_idx)) {
            if (i >= indexes[0] && i < indexes[1]) {
                dataStyles.push({
                    text: text[i],
                    style: { color: colorFromKey(k), fontFamily: 'Courier New, Courier, monospace' },
                });
                continue nextchar;
            }
        }
        ...
        dataStyles.push({
            text: text[i],
            style: { color: render_text_color, fontFamily: 'Courier New, Courier, monospace' },
        });
    }
    return dataStyles;
}
...
const GrokDebugger = () => {
    ...
    const completion = async (context) => {
        ...
        let matchingPatterns = Object.keys(loadedGrokPatterns.current)
            .filter((pattern) => pattern.startsWith(cleanPattern))
            .sort()
            .map((pattern) => ({ label: pattern, type: 'text' }))
            .slice(0, 10);
        return {
            from: word.from + 2,
                ...
                <Box display="flex" justifyContent="center" maxWidth="50%" margin="0 auto">
                    <TableContainer component={Paper}>
                        <Table aria-label="simple table" size="small" width="50%">
                            <TableHead>
                                <TableRow>
                                    <TableCell style={{ fontSize: '1.2em' }} align="right">
                                        {columns[0].title}
                                    </TableCell>
    ...
    const suppressNewline = (tr) => {
        if (tr.newDoc.sliceString(0).split('\n').length > 1) {
            console.log('Found ', tr.newDoc.sliceString(0).split('\n').length, ' newlines');
            console.log('Cutting from', tr.newDoc.sliceString(0).indexOf('\n'), 'to', tr.newDoc.sliceString(0).length);
            return [tr.newDoc.sliceString(0).indexOf('\n'), tr.newDoc.sliceString(0).length];
        }
        return true;
    };

	const commonCodeMirrorExtensions = [
		EditorView.lineWrapping,
		EditorState.changeFilter.of(suppressNewline),
    ...
    return (
        <Grid container spacing={2}>
                    ...
                    <div>
                        ...
                        <CodeMirror
                            ...
                            onChange={(value) => {
                                ...
                                setPatternValue(singleLineValue);
                            }}
                            theme="dark"
                            extensions={[
                                grokLanguage,
                        ...
                        <CodeMirror
                            ...
                            theme="dark"
                            extensions={commonCodeMirrorExtensions}
                            style={{ textAlign: 'left' }}
                        />
                        <ButtonGroup>
                            <IconButton
            ...
            <Grid item xs={4} md={4}>
                <Item>
                    <GrokLibrary onPatternUpdate={refreshGrokPatterns} />
                </Item>
            </Grid>
        </Grid>
    );
