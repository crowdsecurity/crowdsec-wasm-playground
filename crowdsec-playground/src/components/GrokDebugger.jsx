...
const render_text_color = "#D3D3D3";
const render_family_font = "Courier New, Courier, monospace";

const render_hl_not_captured = "#87A26B";
const render_hl_captured = "#395D15";
...
function renderPattern(end_idx, pattern, submatch_idx) {
  ...
  for (let i = 0; i < pattern.length; i++) {
    ...
    if (pattern[i] === "%" && pattern[i + 1] === "{") {
      ...
      submatch_name = pattern.substring(
        capture_name_start + 1,
        pattern.indexOf("}", capture_name_start + 1),
      );
    ...
    if (insubmatch === true) {
      patternStyles.push({
        text: pattern[i],
        style: {
          color: colorFromKey(submatch_name),
...
const GrokDebugger = () => {
  ...
  useEffect(() => {
    ...
    const parseAnchorData = () => {
      ...
      if (anchorEncodedData) {
        ...
        if (
          parsedData["pattern"] === undefined ||
          parsedData["input"] === undefined
        ) {
          return;
        }
  ...
  const handleClick = () => {
    ...
    const idx = ret["__idx"];
    delete ret["__idx"];
    const error = ret["__error"];
    delete ret["__error"];
    const start_idx = ret["__idx_start"];
    ...
    if (idx !== undefined) {
      ...
      setDataStyles(
        renderText(start_idx, end_idx, submatch_indexes, inputValue),
      );
